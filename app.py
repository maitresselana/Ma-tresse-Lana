...
"""
    ...
    send_push_notification(
        "Nouvelle réservation",
        f"{pseudo} — {start.strftime('%d/%m à %H:%M')} — {service['name']}",
        "/admin",
    )
...
def admin():
    ...
    dashboard["week_revenue"] = con.execute(
        "SELECT COALESCE(SUM(price),0) FROM bookings WHERE status='confirmed' AND start_dt>=? AND start_dt<?",
        (dtstr(now - timedelta(days=7)), dtstr(now)),
    ).fetchone()[0]
    dashboard["average_booking"] = con.execute(
        "SELECT COALESCE(AVG(price),0) FROM bookings WHERE status='confirmed'"
    ).fetchone()[0]
    ...
    clients_preview = con.execute(
        """SELECT email, MAX(pseudo) AS pseudo,
           ...
           FROM bookings GROUP BY email ORDER BY last_booking DESC LIMIT 20"""
    ).fetchall()
