...
def admin():
    ...
    counts = {
        ...
        "confirmed": con.execute("SELECT COUNT(*) FROM bookings WHERE status='completed'").fetchone()[0],
    ...
    dashboard = {
        "today_confirmed": con.execute(
            "SELECT COUNT(*) FROM bookings WHERE status='completed' AND start_dt>=? AND start_dt<?",
            (dtstr(today_start), dtstr(today_end)),
        ).fetchone()[0],
        "today_revenue": con.execute(
            "SELECT COALESCE(SUM(price),0) FROM bookings WHERE status='completed' AND start_dt>=? AND start_dt<?",
            (dtstr(today_start), dtstr(today_end)),
        ).fetchone()[0],
    ...
    upcoming_bookings = con.execute(
        "SELECT * FROM bookings WHERE status='completed' AND start_dt>=? ORDER BY start_dt ASC LIMIT 5",
        (dtstr(now),),
    ).fetchall()
    ...
    dashboard["average_booking"] = round(con.execute(
        "SELECT COALESCE(AVG(price),0) FROM bookings WHERE status='completed'"
    ).fetchone()[0])
    ...
    clients_preview = con.execute(
        """SELECT email, MAX(pseudo) AS pseudo,
                  ...
                  SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) AS confirmed_bookings,
...
def admin_clients():
    ...
    clients = con.execute(
        ...
        SELECT
          ...
          COUNT(*) AS total_bookings,
          SUM(CASE WHEN b.status='completed' THEN 1 ELSE 0 END) AS confirmed_bookings,
    ...
    if any(word in question for word in ("demain", "rendez-vous demain", "rdv demain")):
        ...
        amount = con.execute(
            """SELECT COALESCE(SUM(price),0) FROM bookings
               WHERE status='completed' AND start_dt>=? AND start_dt<?""",
            (dtstr(month_start), dtstr(next_month)),
        ).fetchone()[0]
